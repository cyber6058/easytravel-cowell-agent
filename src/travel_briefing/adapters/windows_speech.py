...
class WindowsSpeechAdapter:
    ...
    def synthesize(self, job_path: Path, *, timeout_seconds: int) -> None:
        ...
        if not script.is_file():
            raise AudioSynthesisError("Hanhan PowerShell adapter script is missing")
        if not job.is_file():
            raise AudioSynthesisError("Hanhan speech job file is missing")
        command = [
        ...
        try:
            ...
            raise LocalTtsUnavailableError(
                "Windows PowerShell is unavailable for local TTS"
            ) from error
        if result.returncode == 21:
            raise LocalTtsUnavailableError()
        if result.returncode != 0:
            raise AudioSynthesisError(
                "Local Hanhan synthesis failed",
